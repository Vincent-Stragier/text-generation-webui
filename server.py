            ...
            with gr.Box():
                with gr.Row():
                    with gr.Column():
                        ...
                        shared.gradio["n_gqa"] = gr.Slider(
                            minimum=0,
                            maximum=16,
                            step=1,
                            label="n_gqa",
                            value=shared.args.n_gqa,
                            info="grouped-query attention. Must be 8 for llama2 70b.",
                        )
                        shared.gradio["rms_norm_eps"] = gr.Slider(
                            minimum=0,
                            maximum=1e-5,
                            step=1e-6,
                            label="rms_norm_eps",
                            value=shared.args.n_gqa,
                            info="5e-6 is a good value for llama2 70b.",
                        )

                        shared.gradio["wbits"] = gr.Dropdown(
                            label="wbits",
                            choices=["None", 1, 2, 3, 4, 8],
                            value=str(shared.args.wbits)
    ...
    if not shared.args.multi_user:
        ...
        if shared.is_chat():
            ...
            shared.gradio["session_menu"].change(
                load_session,
                gradio("session_menu", "interface_state"),
                gradio("interface_state"),
            ).then(
                ui.apply_interface_values,
                gradio("interface_state"),
    ...
    with gr.Blocks(
                    ...
                    with gr.Column(scale=4):
                        ...
                        with gr.Row():
                            ...
                            shared.gradio["Stop"] = gr.Button(
                                "Stop", elem_classes="small-button"
                            )
        ...
        if shared.is_chat():
            ...
            shared.gradio["upload_chat_history"].upload(
                chat.load_history,
                gradio("upload_chat_history", "history"),
                gradio("history"),
            ).then(chat.redraw_html, shared.reload_inputs, gradio("display"))
