            ...
            if self.verbose:
                ...
                print(
                    [
                        ...
                        for token_id in self.top_token_ids_list[-1]
                    ]
                )
...
def output_modifier(text):
    ...
    top_tokens_list = [
        [shared.tokenizer.decode(token_id) for token_id in top_token_ids]
    ...
    if params["color_by_probability"] and params["color_by_perplexity"]:
        ...
        i = 0
        for token, ppl, top_tokens, top_probs in zip(
            gen_tokens, perplexities, top_tokens_list, top_probs_list
        ):
            if "`" in token:
                in_code = not in_code
                continue
        ...
        i = 0
        for token, prob, top_tokens, top_probs in zip(
            gen_tokens, sel_probs, top_tokens_list, top_probs_list
        ):
            if "`" in token:
                in_code = not in_code
                continue
