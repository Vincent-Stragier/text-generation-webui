...
def generate_4chan_html(f):
    ...
    for i in range(len(output)):
        ...
        output[i] = re.sub(
            r'^<blockquote class="message">(&gt;(.*?)(<br>|</div>))',
            r'<blockquote class="message"><span class="greentext">\1</span>',
            output[i],
        )
    output = "\n".join(output)
...
def generate_cai_chat_html(history, name1, name2, style, reset_cache=False):
    ...
    img_bot = (
        f'<img src="file/cache/pfp_character.png?{name2}">'
...
def chat_html_wrapper(history, name1, name2, mode, style, reset_cache=False):
    if mode == "instruct":
        ...
        return generate_cai_chat_html(
            history["visible"], name1, name2, style, reset_cache
        )
